...
const InterconnexionForm = ({
    ...
    const [formData, setFormData] = useState({
        ...
        production_totale_recu: 0,
        production_totale_fourni: 0,
        productions: {}
    });
    useEffect(() => {
        ...
        for (let hour in productionHours) {
            productions[productionHours[hour]] = { recu: 0, fourni: 0 };
        }
            ...
            content.push(
                <>
                    <Form.Group as={Col} key={e}>
                        ...
                        />
                        <Form.Label>{e} Fourni</Form.Label>
                        <Form.Control
                            type="number"
                            placeholder="Productions"
    ...
    return (
        <Form onSubmit={e => handleFormSubmit(e, formData)}>
            ...
            {selectedCentrale.horaire === "24" && (
                <Form.Row>
                    <Form.Group as={Col}>
                        <Form.Label>Energie Totale Recu</Form.Label>
                        <Form.Control
                            type="number"
                            placeholder="Energie Totale Recu"
                            value={formData.production_totale_recu}
                            onChange={({ target }) => {
                                if (
                                ...
                                setFormData({
                                    ...formData,
                                    production_totale_recu: target.value
                                });
                            }}
                        />
                    </Form.Group>
                    <Form.Group as={Col}>
                        <Form.Label>Energie Totale Fourni</Form.Label>
                        <Form.Control
                            type="number"
                            placeholder="Energie Totale Fourni"
                            value={formData.production_totale_fourni}
                            onChange={({ target }) => {
                                if (
                                    Number(target.value) < 0 ||
                                    isNaN(target.value) ||
                                ...
                                setFormData({
                                    ...formData,
                                    production_totale_fourni: target.value
                                });
                            }}
                        />
                    </Form.Group>
...
export default InterconnexionForm;
